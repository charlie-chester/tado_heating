...
ROOMS = {
    "conservatory": {"morning": 15, "day": 17, "sun_correction": 2},
    "dining_room": {"morning": 15, "day": 17, "sun_correction": 2},
    "lounge": {"morning": 16, "day": 17, "sun_correction": 0},
...
class TadoHeatControl(hass.Hass):
    ...
    def main(self, kwargs):
        ...
        for room_name, room_data in ROOMS.items():
            ...
            self.log(f"Current weather ID: {current_weather_id}")
            ...
            if sunrise <= datetime.now().time() < datetime.now().time().replace(hour=MORNING_ENDS[0], minute=MORNING_ENDS[1]):
                ...
                self.log(f"Morning: Minimum room temperature set to: {room_minimum_temp} C")
                ...
                if current_weather_id == 800 or current_weather_id == 801:
                    ...
                    self.log(f"Minimum room temperature after sun correction set to: {room_minimum_temp} C")
            else:
                room_minimum_temp = room_data["day"]
                self.log(f"Afternoon / Evening: Minimum room temperature set to: {room_minimum_temp} C")
